...
import jababarium.util.ui.JBUIFunc;
...
public abstract class CommandableAttackerBlock extends CommandableBlock {
    ...
    public abstract class CommandableAttackerBlockBuild extends CommandableBlockBuild {
        ...
        public BlockStatus status() {
            return canCommand(targetVec) ? BlockStatus.active
                    : isCharging() ? BlockStatus.noOutput : BlockStatus.noInput;
        }
        ...
        public void command(Vec2 pos) {
            ...
            JBFx.attackWarningPos.at(lastConfirmedTarget.x, lastConfirmedTarget.y, configureChargeTime, team.color,
                    tile);
        }
        ...
        public void drawConfigure() {
            ...
            for (CommandableBlockBuild build : JBGroups.commandableBuilds) {
                if (build != this && build != null && build.team == team && sameGroup(build.block)
                        && build.canCommand(targetVec)) {
                    builds.add(build);
            ...
            if (canCommand(targetVec))
                builds.add(this);
            if (builds.any())
                DrawFunc.overlayText(Core.bundle.format("mod.ui.participants", builds.size), targetVec.x, targetVec.y,
                        tilesize * 2f, Pal.accent, true);
        }
        ...
        public void commandAll(Vec2 pos) {
            ...
            if (!Vars.headless && participantsTmp.any()) {
                if (team != Vars.player.team())
                    JBUIFunc.showToast(Icon.warning,
                            "[#ff7b69]Caution: []Attack " + (int) (pos.x / 8) + ", " + (int) (pos.y / 8),
                            JBSounds.artilleryFire1);
                JBFx.attackWarningRange.at(pos.x, pos.y, 80, team.color);
            }
        }
