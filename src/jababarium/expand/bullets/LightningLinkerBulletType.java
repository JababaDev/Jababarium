package jababarium.expand.bullets;
...
public class LightningLinkerBulletType extends BasicBulletType {
    ...
    public Sound randomGenerateSound = Sounds.explosionArtilleryShock;
    public Cons<Position> hitModifier = p -> {
    };
    public float range = -1;
    public int maxHit = 60;
    public int boltNum = 1;
    public int effectLingtning = 2;
    public float effectLightningChance = 0.50f;
    ...
    public LightningLinkerBulletType(float speed, float damage) {
        ...
        lightning = 4;
        lightningLength = 3;
        lightningLengthRand = 36;
        lightningCone = 720f;
        lightningDamage = damage;

        trailWidth = -1;

        liHitEffect = JBFx.lightningHitSmall;
    }
    ...
    public void init() {
        super.init();
        if (slopeEffect == null)
            slopeEffect = new Effect(25, e -> {
                if (!(e.data instanceof Integer))
                    return;
                int i = e.data();
                Draw.color(backColor);
                Angles.randLenVectors(e.id, (int) (size / 8f), size / 4f + size * 2f * e.fin(),
                        (x, y) -> Fill.circle(e.x + x, e.y + y, e.fout() * size / 1.65f));
                Lines.stroke((i < 0 ? e.fin() : e.fout()) * 3f);
                Lines.circle(e.x, e.y, (i > 0 ? e.fin() : e.fout()) * size * 1.1f);
            });
        if (spreadEffect == null)
            spreadEffect = new Effect(32f, e -> randLenVectors(e.id, 2, 6 + 45 * e.fin(), (x, y) -> {
                color(backColor);
        ...
        if (trailWidth < 0)
            trailWidth = size * 0.75f;
        if (trailLength < 0)
            trailLength = 12;

        drawSize = Math.max(drawSize, size * 2f);

        if (effectLightningLength < 0)
            effectLightningLength = size * 1.5f;
        if (effectLightningLengthRand < 0)
            effectLightningLengthRand = size * 2f;
    }
    ...
    public void update(Bullet b) {
        ...
        if (b.timer(4, hitSpacing)) {
            ...
            PosLightning.createRange(b, collidesAir, collidesGround, b, b.team, linkRange, maxHit, backColor,
                    Mathf.chanceDelta(randomLightningChance), lightningDamage, lightningLength, PosLightning.WIDTH,
                    boltNum, p -> {
        ...
        if (randomGenerateRange > 0f && Mathf.chance(Time.delta * randomGenerateChance)
            ...
            PosLightning.createRandomRange(b, b.team, b, randomGenerateRange, backColor,
                    Mathf.chanceDelta(randomLightningChance), 0, 0, boltWidth, boltNum, randomLightningNum, hitPos -> {
                        ...
                        JBFx.lightningHitLarge.at(hitPos.getX(), hitPos.getY(), lightningColor);

                        hitModifier.get(hitPos);
                    });

        
        if (b.timer(5, 3f) && b.lifetime - b.time > Fx.chainLightning.lifetime) {
            for (int i = 0; i < effectLingtning; i++) {
                Vec2 vel = new Vec2().set(b.vel).scl(Fx.chainLightning.lifetime / 2);
                Vec2 v = new Vec2().rnd(effectLightningLength + Mathf.random(effectLightningLengthRand))
                        .add(b)
                        .add(vel);
                Fx.chainLightning.at(b.x, b.y, 12f, backColor, v.cpy());
                JBFx.lightningHitSmall.at(v.x, v.y, 20f, backColor);
            }
        }
    }
